#!/usr/bin/env python3
"""Compute execution blockers + no-trade reason for a daily report date."""
...
from quant_exec_blockers import (
    compute_exec_blockers_and_no_trade,
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Compute execution blockers/no-trade outputs.")
...
def main() -> int:
    ...
    rc, info = compute_exec_blockers_and_no_trade(
    ...
    if int(rc) == 2:
        print(f"[ERROR] {info.get('error', 'compute failed')}")
    elif bool(args.verbose):
        print("[PASS] a19_compute_exec_blockers")
    return int(rc)
