...
- 1：现金利用策略升级，支持高置信信号临时降低现金目标（仍保留最低现金底线）
- 2：动量从单周期升级为多周期融合，改善早期趋势识别
- 3：新增高置信单标的权重提升机制（仅单票，且受总风险约束）
- 4：新增持仓退出信号模块（价量形态），支持减仓或清仓模式
- 5：GlobalWatch 增加结构化主题信号，支持可选 LLM 主题注入链路
- 6：新增分数平滑/标准化/截断与组合级风险闸门（波动率与集中度）
